...
(수정: 포켓몬 카드 스타일, 3D Tilt, Holographic 효과 적용)
"""
import sys
...
# --- [스타일링] 카드 디자인을 위한 CSS 및 JavaScript ---
...
@keyframes shine {
    0% { transform: translateX(-60%) translateY(-10%) rotate(20deg); }
    100% { transform: translateX(60%) translateY(10%) rotate(20deg); }
}
/* Dead Player Styling */
.card.dead {
    filter: grayscale(100%);
}
/* Broken Glass Effect for Dead players */
.card.dead::before {
    content: '';
    position: absolute;
    top: 0; left: 0;
    ...
    background: 
        linear-gradient(to top right, rgba(255,255,255,0) 0%, rgba(255,255,255,0) 49.8%, rgba(255,255,255,0.4) 50%, rgba(255,255,255,0) 50.2%),
...
async def main_page(client: Client):
    """메인 웹 페이지를 구성합니다."""
    # CSS 및 JS 주입
    ui.add_head_html(f'<style>{CARD_CSS}</style>')
