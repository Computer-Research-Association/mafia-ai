...
def get_random_narrative(event: GameEvent) -> str:
    ...
    if event.event_type == EventType.CLAIM:
        if event.value == Role.POLICE and (
            event.target_id is None or event.target_id == event.actor_id
        ):
            key = "CLAIM_SELF_POLICE"
        elif event.value == Role.DOCTOR and (
            event.target_id is None or event.target_id == event.actor_id
        ):  # New logic
            key = "CLAIM_SELF_DOCTOR"
        elif (
            event.value == Role.MAFIA
    ...
    if key and key in narrative_variations:
        ...
        return (
            template.format(target_id=event.target_id)
            if "{target_id}" in template
            else template
        )
...
def update_ui_for_game_state(client: Client):
    ...
    for player in state.game_engine.players:
        client.run_javascript(
            f"document.getElementById('player-role-{player.id}').innerText = '{player.role.name}';"
        )
        client.run_javascript(
            f"document.getElementById('player-card-{player.id}').classList.{'add' if not player.alive else 'remove'}('dead');"
        )
...
def process_ui_events(client: Client):
    ...
    if event_type == "announcement":
        ...
        js_call = (
            f"type_text('player-bubble-text-{actor_id}', '{text}', {hold_duration_ms})"
        )
...
async def step_phase_handler(client: Client):
    ...
    await ui.run_javascript("window.playMainBgm();")
    if state.game_over:
    ...
    if human_player.alive:
        ...
        elif old_phase == Phase.NIGHT and human_player.role in [
            Role.MAFIA,
            Role.POLICE,
            Role.DOCTOR,
        ]:
    ...
    if requires_human_action:
        ...
        update_modal_ui()
        client.run_javascript(
            'document.getElementById("human-control-modal").classList.add("visible");'
        )
    ...
    if state.previous_phase != old_phase:
        if old_phase == Phase.NIGHT:
            ...
            state.ui_event_queue.append(
                ("announcement", f"{state.game_engine.day}일차 낮이 밝았습니다")
            )
    ...
    for event in new_events:
        ...
        if event.event_type == EventType.EXECUTE:
            state.pending_death_announcements.append(
                f"투표로 {event.target_id}번 플레이어가 처형되었습니다. (직업: {event.value.name})"
            )
        elif event.event_type == EventType.KILL:
            state.pending_death_announcements.append(
                f"지난 밤 {event.target_id}번 플레이어가 살해당했습니다."
            )
        elif (
            event.event_type in [EventType.CLAIM, EventType.VOTE]
    ...
    if dom_order:
        for player_id in dom_order:
            if player_id in new_narrative_events:
                state.ui_event_queue.append(
                    ("narrative", player_id, new_narrative_events[player_id])
                )
...
def on_execution_vote(agree: bool, target_id: Optional[int]):
    """처형 찬반 투표를 처리하는 콜백"""
    ui.run_javascript(
        'document.getElementById("human-control-modal").classList.remove("visible");'
    )
...
def select_player_action(player_id: int):
    ...
    if current_phase == Phase.DAY_DISCUSSION:
        ...
        state.current_instruction = (
            f"Player {player_id}에 대해 역할을 주장하거나, 주장 없이 의견만 피력합니다."
        )
        update_modal_ui()
    else:
    ...
    if target_id is not None:
        ...
        with ui.row().classes("w-full gap-4 justify-center items-center"):
            with ui.card().classes(
                "bg-red-800 w-48 h-48 cursor-pointer hover:bg-red-700"
            ).on("click", lambda t=target_id: on_execution_vote(True, t)):
                with ui.column().classes("w-full h-full justify-center items-center"):
                    ui.label("찬성").classes("text-white text-4xl font-bold")

            with ui.card().classes(
                "bg-blue-800 w-48 h-48 cursor-pointer hover:bg-blue-700"
            ).on("click", lambda t=target_id: on_execution_vote(False, t)):
                with ui.column().classes("w-full h-full justify-center items-center"):
                    ui.label("반대").classes("text-white text-4xl font-bold")
    else:
        # 처형 대상이 없는 경우 (동점자 발생 등)
        state.current_instruction = (
            "처형 대상이 결정되지 않았습니다. 다음 단계로 넘어갑니다."
        )
...
def render_player_selection():
    ...
    if current_phase == Phase.DAY_DISCUSSION:
        ...
        if human_player.role == Role.MAFIA:
            phase_msg = (
                f"밤: 제거할 플레이어를 선택하세요. (당신은 {human_player.role.name})"
            )
        elif human_player.role == Role.POLICE:
            phase_msg = (
                f"밤: 조사할 플레이어를 선택하세요. (당신은 {human_player.role.name})"
            )
        elif human_player.role == Role.DOCTOR:
            phase_msg = (
                f"밤: 보호할 플레이어를 선택하세요. (당신은 {human_player.role.name})"
            )
    ...
    with ui.element("div").classes("action-button-grid grid-cols-4"):
        for player in state.game_engine.players:
            ...
            with ui.card().classes(card_classes).on(
                "click", lambda pid=player.id: select_player_action(pid)
            ) as card:
                if not is_alive:
                    card.props("disable")
    ...
    ui.button("기권/패스", on_click=lambda: on_player_or_role_action(-1, None)).classes(
        "w-full mt-4 utility-button back"
    )


def render_role_selection():
    """역할 선택 UI 렌더링"""
    ui.label(f"Player {state.selected_target}를 선택했습니다").classes(
    ...
    with ui.element("div").classes("action-button-grid grid-cols-4"):
        for role in [Role.POLICE, Role.DOCTOR, Role.MAFIA, Role.CITIZEN]:
            btn = ui.button(
                f"{role.name}", on_click=lambda r=role: select_role_action(r)
            ).classes("action-button role-claim-button")

    with ui.element("div").classes("utility-button-grid"):
        ui.button("역할 주장 안 함", on_click=lambda: select_role_action(None)).classes(
            "utility-button confirm"
        )
...
async def main_page(client: Client):
    ...
    with ui.row().classes("w-full h-16 items-center justify-between z-30").style(
        "padding: 1rem 4rem; background: transparent;"
    ):
        ui.label().bind_text_from(state, "day_phase_text").classes("text-xl").style(
            'color: rgba(26, 26, 26, 0.85); font-weight: 500; letter-spacing: 1px; font-family: "Inter", "Noto Sans KR", sans-serif;'
        )
        next_button = (
            ui.button(on_click=lambda: step_phase_handler(client))
            .props("flat")
            .classes("px-8 py-2")
        )
        ...
        next_button.bind_enabled_from(
            state,
            "is_processing_events",
            backward=lambda v: not v and not state.waiting_for_human,
        )
        next_button.style(
            'background: rgba(26, 26, 26, 0.08); border: 1px solid rgba(26, 26, 26, 0.15); border-radius: 8px; font-weight: 500; letter-spacing: 0.5px; font-family: "Inter", "Noto Sans KR", sans-serif; color: rgba(26, 26, 26, 0.85); transition: all 0.2s ease; text-transform: none;'
        )

    with ui.element("div").props('id="human-control-modal"').classes(
        "fixed inset-0 flex items-center justify-center z-50"
    ) as human_control_modal:
        with ui.column().classes("items-center gap-4 w-full max-w-2xl"):
            ui.label("당신의 차례입니다!").classes(
                "action-modal-header text-3xl font-bold text-center mb-2"
            )

            role_label = ui.label().classes(
                "action-modal-role text-lg text-center mb-4"
            )
            role_label.bind_text_from(
                state.game_engine,
                "players",
            ...
            ui.separator().style(
                "background: rgba(255, 255, 255, 0.2); margin: 1rem 0;"
            )

            instruction_label = ui.label().classes(
                "action-modal-instruction text-xl font-semibold text-center mb-6"
            )
            instruction_label.bind_text_from(state, "current_instruction")

            global modal_container
            with ui.column().classes("w-full modal-content-wrapper").props(
    ...
    with ui.element("div").classes("player-area w-full"):
        # 영역 컨테이너 (보이는 레이아웃)
        with ui.element("div").props('id="area-container"').style(
            "display: flex; width: 100%; height: 100%; justify-content: space-between;"
        ):
            # 왼쪽: 살아있는 플레이어들
            with ui.element("div").props('id="alive-deck"').style("flex: 0 0 60%;"):
                for i in range(8):
                    ui.element("div").props(
                        f'id="placeholder-{i}" class="card-placeholder"'
                    )

            # 오른쪽: 투표/죽은 플레이어
            with ui.element("div").props('id="right-side"').style(
                "flex: 0 0 37.5%; display: flex; flex-direction: column; gap: 2rem;"
            ):
                ui.element("div").props('id="voted-area"').style("flex: 1;")
                ui.element("div").props('id="dead-area"').style("flex: 1;")

        # 카드 레이어 (absolute overlay)
        with ui.element("div").props('id="card-layer"').style(
...
def get_player_statements(player_id: int):
    ...
    for event in state.game_engine.history:
        if (
            ...
            phase_name = (
                event.phase.name.replace("_", " ").title() if event.phase else "Unknown"
            )
