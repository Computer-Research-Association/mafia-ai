...
class RuleBaseAgent(BaseAgent):
        ...
        for event in status.action_history:
            ...
            if event.event_type == EventType.CLAIM:
                if event.target_id == event.actor_id or event.target_id == -1:
                    if event.value:
                        claims[event.actor_id] = event.value
    ...
    def _pick_fair_deterministic(
        self, candidates: List[int], day: int, seed: int
    ) -> int:
        """
        [공정한 결정론적 선택]
    ...
    def _get_confirmed_players(
        ...
        for role_type in [Role.POLICE, Role.DOCTOR]:
            alive_claimants = [
                pid
                for pid, r in claims.items()
                if r == role_type and pid in alive_players
            ]
    ...
    def _get_logic_target(
        self, alive_players: List[int], context: dict, day: int
    ) -> Optional[int]:
        seed = context["seed"]
        ...
        for role_type in [Role.POLICE, Role.DOCTOR]:
            ...
            true_role_dead = [
                pid for pid in claimants if context["known_roles"].get(pid) == role_type
            ]
            if true_role_dead:
                for suspect in claimants:
                    if suspect not in true_role_dead and suspect in alive_players:
                        if suspect != self.id:
                            confirmed_fake.append(suspect)
        if confirmed_fake:
        ...
        police_claims = [
            pid
            for pid, role in context["claims"].items()
            if role == Role.POLICE and pid in alive_players
        ]
        ...
        doctor_claims = [
            pid
            for pid, role in context["claims"].items()
            if role == Role.DOCTOR and pid in alive_players
        ]
    ...
    def _act_vote(
        self, status: GameStatus, context: dict, alive_others: List[int]
    ) -> GameAction:
        alive_players = [p.id for p in status.players if p.alive]
        seed = context["seed"]
        ...
        if len(alive_players) <= 4:
            ...
            if candidates:
                return GameAction(
                    target_id=self._pick_fair_deterministic(
                        candidates, status.day, seed
                    )
                )
            elif alive_others:
                return GameAction(
                    target_id=self._pick_fair_deterministic(
                        alive_others, status.day, seed
                    )
                )
    ...
    def _act_execute(self, status: GameStatus) -> GameAction:
        ...
        for e in status.action_history:
            if (
                e.day == status.day
                and e.phase == Phase.DAY_VOTE
                and e.event_type == EventType.VOTE
            ):
        ...
        if target_id == self.id or target_id == -1:
            return GameAction(target_id=self.id)

        return GameAction(target_id=target_id)

    def _act_night(
        self, status: GameStatus, alive_others: List[int], context: dict
    ) -> GameAction:
        seed = context["seed"]
        if self.role == Role.MAFIA:
            special_roles = [
            ...
            if silent_citizens:
                return GameAction(
                    target_id=self._pick_fair_deterministic(
                        silent_citizens, status.day, seed
                    )
                )
            ...
            if valid:
                return GameAction(
                    target_id=self._pick_fair_deterministic(valid, status.day, seed)
                )
            return GameAction(target_id=-1)
        ...
        if self.role == Role.POLICE:
            rivals = [
                pid
                for pid, role in context["claims"].items()
                if role == Role.POLICE and pid != self.id and pid in alive_others
            ]
            if rivals:
                return GameAction(
                    target_id=self._pick_fair_deterministic(rivals, status.day, seed)
                )

            doctor_claims = [
                pid
                for pid, role in context["claims"].items()
                if role == Role.DOCTOR and pid in alive_others
            ]
            if len(doctor_claims) >= 2:
                uninvestigated_docs = [
                    t for t in doctor_claims if t not in context["investigations"]
                ]
                if uninvestigated_docs:
                    return GameAction(
                        target_id=self._pick_fair_deterministic(
                            uninvestigated_docs, status.day, seed
                        )
                    )
            ...
            uninvestigated = [
                ...
                if t not in context["investigations"] and t not in confirmed
            ]

            if uninvestigated:
                return GameAction(
                    target_id=self._pick_fair_deterministic(
                        uninvestigated, status.day, seed
                    )
                )
            return GameAction(
                target_id=self._pick_fair_deterministic(alive_others, status.day, seed)
            )

        if self.role == Role.DOCTOR:
            dead_real_police = [
                pid for pid, r in context["known_roles"].items() if r == Role.POLICE
            ]
            if dead_real_police:
                return GameAction(target_id=self.id)
            ...
            police_claims = [
                pid
                for pid, role in context["claims"].items()
                if role == Role.POLICE and pid in alive_others
            ]
            if police_claims:
                return GameAction(
                    target_id=self._pick_fair_deterministic(
                        police_claims, status.day, seed
                    )
                )
